...
// Isolated feature block — does not touch save()/sync logic above.
// Order creation + signature verification both happen server-side via
// Supabase Edge Functions ('create-razorpay-order', 'verify-razorpay-payment')
// so the Razorpay Key Secret never has to live in this client-side file.
//
// TODO (only thing left to configure): set RAZORPAY_KEY_ID below to your
// actual Razorpay *publishable* Key ID (safe to expose client-side — starts
// with rzp_test_ or rzp_live_). The Key SECRET goes only into the Supabase
// Edge Function secrets (never here) — see the deployment notes shared
// alongside this code.
const RAZORPAY_KEY_ID = 'rzp_test_YOUR_KEY_ID_HERE';

let _coffeeAmount = 99;

function openBuyCoffee(){
  const mo = document.getElementById('modal-buyCoffee');
  if(!mo) return;
  document.getElementById('coffee-custom-amt').value = '';
  document.getElementById('coffee-amt-err').textContent = '';
  document.querySelectorAll('.coffee-amt-pill').forEach(p=>p.classList.remove('active'));
  const defaultPill = document.querySelector('.coffee-amt-pill[data-amt="99"]');
  if(defaultPill) defaultPill.classList.add('active');
  _coffeeAmount = 99;
  _updateCoffeeBtnLabel();
  mo.classList.add('open');
}

function selectCoffeeAmount(amt, btnEl){
  _coffeeAmount = amt;
  document.getElementById('coffee-custom-amt').value = '';
  document.getElementById('coffee-amt-err').textContent = '';
  document.querySelectorAll('.coffee-amt-pill').forEach(p=>p.classList.remove('active'));
...
function onCoffeeCustomInput(inputEl){
  const v = parseInt(inputEl.value, 10);
  document.querySelectorAll('.coffee-amt-pill').forEach(p=>p.classList.remove('active'));
  document.getElementById('coffee-amt-err').textContent = '';
  _coffeeAmount = (v && v > 0) ? v : 0;
  _updateCoffeeBtnLabel();
}

function _updateCoffeeBtnLabel(){
  const txt = document.getElementById('coffee-pay-btn-txt');
  if(txt) txt.textContent = _coffeeAmount > 0 ? `Pay ₹${_coffeeAmount}` : 'Pay';
}

async function payCoffee(){
  const errEl = document.getElementById('coffee-amt-err');
  if(!_coffeeAmount || _coffeeAmount < 1){
    errEl.textContent = 'Enter a valid amount';
    return;
  }
  if(_coffeeAmount > 100000){
    errEl.textContent = 'Amount too large — please enter a smaller value';
    return;
  }
  if(!sb){
    toast('Please sign in to donate', 'error');
    return;
  }
  if(typeof Razorpay === 'undefined'){
    toast('Payment gateway still loading — try again in a moment', 'error');
    return;
  }

  const payBtn = document.getElementById('coffee-pay-btn');
  const payBtnTxt = document.getElementById('coffee-pay-btn-txt');
  payBtn.disabled = true;
  const prevLabel = payBtnTxt.textContent;
  payBtnTxt.textContent = 'Preparing…';

  try{
    // 1. Ask our Edge Function to create the Razorpay order (server-side,
    //    so the amount can't be tampered with from the client).
    const { data, error } = await sb.functions.invoke('create-razorpay-order', {
      body: { amount: _coffeeAmount }
    });
    if(error || !data || data.error) throw new Error((data && data.error) || error?.message || 'Order creation failed');
    ...
    const rzp = new Razorpay({
      ...
      handler: async function(response){
        try{
          ...
          const { data: verifyData, error: verifyError } = await sb.functions.invoke('verify-razorpay-payment', {
            body: {
          ...
          toast('🎉 Thank you for the coffee! Means a lot.', 'success');
          closeM('buyCoffee');
        }catch(e){
          console.error('Coffee verify error:', e);
          toast('Payment received but verification failed — contact support with your payment ID', 'error');
        }
      },
      modal: {
        ondismiss: function(){
          ...
          payBtnTxt.textContent = prevLabel;
        }
      }
    });
    ...
    console.error('Coffee order error:', e);
